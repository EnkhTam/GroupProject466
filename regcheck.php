...
/*require("/home/data/www/z1865285/public_html/bug/PHPMailer/src/PHPMailer.php");
require("/home/data/www/z1865285/public_html/bug/PHPMailer/src/SMTP.php");
require("/home/data/www/z1865285/public_html/bug/PHPMailer/src/Exception.php");
		...
		if((!empty($usern) || !empty($birth) || !empty($oldpass)) && empty($sqans)){
			if(empty($usern) || empty($birth) || empty($oldpass)){
				...
				if($verifyrow['user_name'] != $usern || $verifyrow['birth_date'] != $birth || !in_array($oldpass,$vwarr))
				{
					...
					{
						if(confirm("You've used this password before, are you sure you wanna use it again?"))
						{
...
if((isset($_POST['pwr']) || $help = true) && !isset($_POST['user_name']))
{
	...
	if($pwrrow == true){
		...
		$link = "<a href='http://students.cs.niu.edu/~z1865285/bug/regcheck.php?key='".$email."'>Click To Reset Password</a>";
...
mysqli_select_db($conn, 'z1865285');
	...
	elseif(mysqli_num_rows($checkemail) == true){
		...
		if($passw == $conpass){
			...
			VALUES ('$birth', '$firstname', '$lastname', '$user', '$passw','$email')");
