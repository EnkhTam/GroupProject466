...
//Senses when user submits a food to the database and does the following
	if(isset($_POST['submitF'])){
		...
		$sqll = "SELECT COUNT(*) AS xx FROM food WHERE food_name = ?";
		$checkfood = $pdo->prepare($sqll);
		$checkfood->execute([$Dfood_name]);
		$checkfoodr = $checkfood->fetch();
		if($checkfoodr['xx'] > 0)
		{
			$message = "Food name already exists";
			echo "<script> alert('$message')</script>";
		}
		elseif(!($checkfoodr['xx'] > 0))
		{
...
//			mysqli_stmt_close($prep);
			...
			$sql = "INSERT INTO food (food_name, serving_size, serving_unit_id, calories, protein, carbs, fiber)
					VALUES (?, ?, ?, ?, ?, ?, ?)";
			$pdo->prepare($sql)->execute([$Dfood_name, $Dserving_size, $Munit, $Dcalories, $Dprotein, $Dcarbs, $DFiber]);
