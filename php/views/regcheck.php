...
mysqli_select_db($conn, 'z1865285');
	...
	elseif(mysqli_num_rows($checkemail) == true){
		...
		//if it all matches the account is created
			...
			*/
				//pdo ver
			$sql = "INSERT INTO user(birth_date, first_name, last_name, user_name, pass_word, email, mset_id, wset_id) 
					VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
			$pdo->prepare($sql)->execute([$birth, $firstname, $lastname, $user, $passw, $email,28,33]);
		...
		//password history entry
			...
			*/
				//pdo ver
			$sql = "INSERT INTO pw_history(user_id, oldpw)
					VALUES(?,?)";
			$pdo->prepare($sql)->execute([$uidrow['user_id'],$passw]);
		//security question entry
			...
			*/
			$sql = "SELECT COUNT(*) AS xx FROM security_questions WHERE sq_text = ?";
			$wentitxtres = $pdo->prepare($sql);
			$wentitxtres->execute([$wenti]);
			//$wentirows = $wentitxtres->fetchAll();
			$wentirow = $wentitxtres->fetch();
				/*$wentiarr = [];
			...
			if(!($wentirow['xx'] > 0))
			{
				$sql = "INSERT INTO security_questions(sq_text)
						VALUES(?)";
				$pdo->prepare($sql)->execute([$wenti]);
			}
			$sql = "SELECT * FROM security_questions WHERE sq_text = ?";
			$wentitxtres = $pdo->prepare($sql);
			$wentitxtres->execute([$wenti]);
			$wentirow = $wentitxtres->fetch();

			$sql = "INSERT INTO sq_user(user_id,sq_id,answer) 
					VALUES(?,?,?)";
			$pdo->prepare($sql)->execute([$uidrow['user_id'],$wentirow['sq_id'],$daan]);
			/*$wentitxtres = mysqli_query($conn, "SELECT * FROM security_questions WHERE sq_text = '$wenti'");
			$wentirow = mysqli_fetch_array($wentitxtres);
