...
/*mysqli_query($conn, "CREATE TABLE measurement_conv(
	   ...
	   from_unit_offset DECIMAL(20,20) NOT NULL,
	   mult DECIMAL(20,20) NOT NULL,
	   divi DECIMAL(20,20) NOT NULL,
	   to_unit_offset DECIMAL(20,20) NOT NULL,
	   PRIMARY KEY (type_id, from_unit_id, to_unit_id),
	   FOREIGN KEY (type_id) REFERENCES measurement_type(type_id),
	   FOREIGN KEY (from_unit_id) REFERENCES measurement_unit(unit_id),
	   FOREIGN KEY (to_unit_id) REFERENCES measurement_unit(unit_id)
...
/*mysqli_query($conn, "INSERT INTO measurement_conv(type_id, from_unit_id, to_unit_id, mult, divi)
			VALUES (4, 28, 29, 0.001, 1000), (4, 28, 30, 0.000001, 1000000), (4, 28, 31, 0.0000000001, 1000000000),
				   (4, 28, 32, 0.00003527336, 28350), (4, 32, 33, 0.0625, 16), (4, 32, 34, 0.0046, 224),
				   (4, 32, 35, 0.000558036, 50802345), (4, 32, 36, 0.000027902, 35840)");*/
mysqli_query($conn, "INSERT INTO measurement_conv(type_id, from_unit_id, to_unit_id, mult, divi)
...
/*$cnt = 1;
$ct = 0;
$conres = mysqli_query($conn, $consql);
do{
	if(($conres != 0) && !($cnt === 0) && ($cnt < 47) && ($cnt > 0)){
		$cnt = mysqli_num_rows($conres);
	}
	/*if(!($cnt === 0) && ($cnt < 47) && ($cnt > 0)){
		continue;
	}*
	else{
		break;
	}
}while(!($cnt === 0) && ($cnt < 47) && ($cnt > 0));
$conres = mysqli_query($conn, $consql);
while(($conrow = mysqli_num_rows($conres) != 0) && ($conres != 0)){
	$conres = mysqli_query($conn, $consql);
}*/
mysqli_query($conn, "UPDATE TABLE measurement_conv SET mult = 1, divi = 1 WHERE mult = divi");
?>
